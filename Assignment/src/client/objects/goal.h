...
public:
    ...
    void _ready() {
        ...
        Color emit;
        switch(this->color) {
        case RED: emit = Color(255, 0, 0, 255); break;
        case BLUE: emit = Color(0, 0, 255, 255); break;
        default: return;
        }
        mat->set_emission(emit);
        mesh->set_surface_material(0, mat);

        this->get_node("./Mesh/Area")->connect("body_entered", this, "body_entered");
    }
    void _process(real_t t) { Entity::entity_process(this->entity_id); }
    void _physics_process(real_t t) { }

    void body_entered(Node *other) {
        if(other->has_method("entered_goal")) other->call("entered_goal", this);
    }
    ...
    void score(const char *object_name, uint32_t worth, uint32_t num) {
        ...
        switch(this->color) {
        case RED: teamcolor = "Red"; break;
        case BLUE: teamcolor = "Blue"; break;
        default: return;
        };
        ...
        if(score) score->set_text(String::num_int64(score->get_text().to_int() + num * worth) + "\n");
        if(counter) counter->set_text(String::num_int64(counter->get_text().to_int() + num) + "\n");
    }
