...
Chunk::Chunk() {
    material = ResourceLoader::get_singleton()->load(
        "res://assets/materials/ground.tres");
}
...
void Chunk::create(int32_t x, int32_t y, OpenSimplexNoise *noise,
                   bool thread_safe) {
    _x = x;
    _y = y;
    ...
    for (int32_t i = 0; i <= size; i++) {
        for (int32_t j = 0; j <= size; j++) {
            this->heightmap[i][j] =
                noise->get_noise_2d((real_t)x + i, (real_t)y + j) * height;
        }
    }
    if (!thread_safe) {
...
void Chunk::mesh() {
    for (int i = 0; i < size; i += resolution_divider) {
        for (int j = 0; j < size; j += resolution_divider) {
            ...
            pool.push_back(Vector3((real_t)i + resolution_divider,
                                   heightmap[i + resolution_divider][j],
                                   (real_t)j));
            pool.push_back(Vector3((real_t)i,
                                   heightmap[i][j + resolution_divider],
                                   (real_t)j + resolution_divider));
            ...
            pool.push_back(Vector3((real_t)i + resolution_divider,
                                   heightmap[i + resolution_divider][j],
                                   (real_t)j));
            pool.push_back(Vector3(
                (real_t)i + resolution_divider,
                heightmap[i + resolution_divider][j + resolution_divider],
                (real_t)j + resolution_divider));
            tool->add_triangle_fan(pool);
        }
    }
...
Chunk *Chunk::chunkAt(int32_t x, int32_t y, OpenSimplexNoise *noise) {
    ...
    if (x < this->_x || (x == this->_x && y < this->_y)) {
        ...
        auto c = Chunk::allocated.back();
        Chunk::allocated.pop_back();
